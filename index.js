...
const logger = {
    ...
    banner: () => {
        ...
        console.log(`---------------------------------------------`);
        console.log(` Neura Bot - Insiders - Reup WangMinHei `);
        console.log(`---------------------------------------------${colors.reset}\n`);
    },
};

// Modified: Add randomDelay function for 10-15 seconds
const delay = (ms) => new Promise((r) => setTimeout(r, ms));
const randomDelay = () => {
    const ms = Math.floor(Math.random() * (15000 - 10000 + 1)) + 10000; // Random between 10000 and 15000 ms
    return delay(ms);
};
    ...
    async performSwapWithRetries(tokenIn, tokenOut, amountInStr, maxRetries = 3) {
        for (let i = 0; i < maxRetries; i++) {
            try {
                ...
                logger.warn(`Attempt ${i + 1}/${maxRetries} failed: ${message}. Retrying after random delay...`);
                // Modified: Replaced await delay(10000) with randomDelay()
                await randomDelay();
            }
        }
    ...
    for (const pk of pks) {
        ...
        try {
            for (let j = 0; j < repeats; j++) {
                ...
                if (swapSuccess) {
                    logger.loading('Waiting random delay before swapping back...');
                    // Modified: Replaced await delay(10000) with randomDelay()
                    await randomDelay();
                ...
                logger.loading('Waiting random delay before next wallet/cycle...');
                // Modified: Replaced await delay(10000) with randomDelay()
                await randomDelay();
            }
        } catch (e) {
